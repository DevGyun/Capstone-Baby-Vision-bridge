...
[최초/재실행]
1. bridge_id 로드 → GET /bridges/{id}/status
2. paired면 → stream_url 받아 바로 송출
3. 아니면 → POST /bridges/register → 6자리 코드 출력 → 앱이 코드 입력할 때까지 폴링 → 송출

* BLE(와이파이 프로비저닝)는 와이파이가 없을 때 start.py가 담당한다. 여기서는 다루지 않는다.
"""
...
def check_status(bridge_id: str) -> dict | None:
    try:
        resp = requests.get(f"{SERVER_URL}/bridges/{bridge_id}/status", timeout=5)
...
def start_streaming(cap, stream_url: str, width: int, height: int, fps: int):
    ...
    try:
        ...
        print("❌ FFmpeg를 찾을 수 없습니다. (sudo apt-get install ffmpeg)")
        cap.release()
        sys.exit(1)
